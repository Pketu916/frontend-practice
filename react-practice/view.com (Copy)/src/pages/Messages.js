...
const Messages = () => {
  ...
  useEffect(() => {
    const fetchOtherUserDetails = async () => {
      ...
      if (Number.parseInt(otherUserId) === user.id) {
        navigate("/messages")
        return
      }

      try {
        const data = await userAPI.getUser(Number.parseInt(otherUserId))
        setOtherUser(data)
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        ...
        const unreadMessageIds = data.filter((msg) => msg.toUserId === user.id && !msg.read).map((msg) => msg.id)
